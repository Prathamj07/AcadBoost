...
def verify_certificate(student_email, certificate_id, teacher_email=None, verification_notes=None):
    ...
    for i, certificate in enumerate(certificates[student_email]):
        if certificate["certificate_id"] == certificate_id:
            ...
            certificates[student_email][i]["verified_by"] = teacher_email
            certificates[student_email][i]["verification_notes"] = verification_notes
            certificates[student_email][i]["verification_status"] = "verified"
...
def get_student_feed(student_email):
    """Get the social feed for a student's department."""
    users = get_users()
    certificates = get_certificates()
    ...
    department_students = [
        ...
        if user.get("role") == "student" and user.get("department") == department
    ]
    ...
    for student_email in department_students:
        if student_email in certificates:
            for cert in certificates[student_email]:
                if cert.get("verified", False):
                    feed_items.append({
                        "type": "certificate",
                        "student_email": student_email,
                        "student_name": users[student_email].get("name", "Unknown"),
                        "title": cert.get("title", "No Title"),
                        "organization": cert.get("issuing_organization", "Unknown"),
                        "issue_date": cert.get("issue_date", ""),
                        "verified_at": cert.get("verified_at", ""),
                        "verified_by": cert.get("verified_by", ""),
                        "verification_notes": cert.get("verification_notes", ""),
                        "timestamp": cert.get("verified_at", "")
                    })
