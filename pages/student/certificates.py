...
def show_my_certificates(email):
    ...
    for certificate in student_certificates:
        certificate_data.append({
            ...
            "Verified By": certificate.get("verified_by", "Not verified yet"),
            "Verification Notes": certificate.get("verification_notes", "No notes"),
    ...
    st.markdown("### Certificate Verification Status")
    ...
    for certificate in student_certificates:
        if not certificate.get("verified", False):
            ...
            st.markdown(f"#### {title}")
            st.markdown(f"**Status:** Pending Verification")
            st.markdown(f"**Submitted On:** {format_datetime(certificate.get('submitted_at', ''))}")
            ...
            st.markdown(f"#### {certificate.get('title', 'No Title')}")
            st.markdown(f"**Status:** Verified")
            st.markdown(f"**Verified By:** {certificate.get('verified_by', 'Unknown')}")
            st.markdown(f"**Verified On:** {format_datetime(certificate.get('verified_at', ''))}")
            st.markdown(f"**Verification Notes:** {certificate.get('verification_notes', 'No notes')}")
