    ...
    if st.session_state.show_resume_form:
        with st.form("resume_form"):
            ...
            for i in range(num_projects):
                ...
                project = {
                    "name": st.text_input(f"Project Name", key=f"proj_name_{i}"),
                    "period": st.text_input(f"Period (e.g., Jan 2023 - Mar 2023)", key=f"proj_period_{i}"),
                    ...
                    "url": st.text_input(f"Project URL (optional)", key=f"proj_url_{i}")
                }
                resume_data["projects"].append(project)
            
            # Skills section
            st.write("### Skills")
            skills_categories = ["Programming Languages", "Frameworks", "Tools", "Soft Skills"]
            
            for category in skills_categories:
                skills = st.text_input(f"{category} (comma-separated)", key=f"skills_{category.lower().replace(' ', '_')}")
                if skills:
                    resume_data["skills"][category] = [skill.strip() for skill in skills.split(",")]
            
            # Submit button
            if st.form_submit_button("Generate Resume", use_container_width=True):
                try:
                    # Generate LaTeX resume
                    latex_path = generate_latex_resume(resume_data)
                    
                    # Convert to PDF
                    pdf_path = os.path.join(os.path.dirname(resume_path), f"{email.replace('@', '_').replace('.', '_')}_resume.pdf")
